    ...
    return (
      <div className={`border-b border-dashed border-[#e5e0d5] py-4 flex gap-4 items-start last:border-none ${state.status === 'correct' ? 'bg-emerald-50/20' : ''}`}>
        ...
        <div className="flex-1">
          ...
          {isDone && <div className={`mt-1 text-[13px] font-medium ${state.status === 'correct' ? 'text-[#2a9d8f]' : 'text-[#e63946]'}`}>{state.status === 'correct' ? '✓ ¡Perfecto!' : `✗ Correct: ${correctAns}`}</div>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f3] text-[#1a1a2e] pb-20 font-sans">
      <div className="bg-[#f4a261] text-white px-10 py-12 pb-10">
        <h1 className="font-extrabold text-4xl mb-4 unbounded">Глагол TENER</h1>
      </div>
      <div className="max-w-[860px] mx-auto px-6">
        ...
        <div className="bg-white border border-[#e5e0d5] rounded-2xl overflow-hidden mb-4 shadow-sm mt-8 px-6">
            <ExerciseItem id="ex1" num="1." problem="Yo _______ un coche rojo. (имею)" correctAns="tengo" />
            <ExerciseItem id="ex2" num="2." problem="¿Tú _______ hermanos? (имеешь)" correctAns="tienes" />
            {/* ... */}
        </div>
      </div>
    </div>
