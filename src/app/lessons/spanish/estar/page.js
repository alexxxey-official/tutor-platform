...
export default function EstarLesson() {
  ...
  const total = 8
  const { progress, updateProgress, getStats, loading } = useLessonProgress('spa_estar', total, 0);
    ...
    return (
      <div className={`border-b border-dashed border-[#e5e0d5] py-4 flex gap-4 items-start last:border-none ${state.status === 'correct' ? 'bg-emerald-50/20' : ''}`}>
        ...
        <div className="flex-1">
          <div className="text-[15px] px-3.5 py-2.5 bg-gray-50 rounded-lg mb-2.5">{problem}</div>
          ...
          {isDone && <div className={`mt-1 text-[13px] font-medium ${state.status === 'correct' ? 'text-[#2a9d8f]' : 'text-[#e63946]'}`}>{state.status === 'correct' ? '✓ ¡Muy bien!' : `✗ Correct: ${correctAns}`}</div>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f3] text-[#1a1a2e] pb-20 font-sans">
      <div className="bg-[#f4a261] text-white px-10 py-12 pb-10">
        <h1 className="font-extrabold text-4xl mb-4 unbounded">Глагол ESTAR</h1>
      </div>
      <div className="max-w-[860px] mx-auto px-6">
        <nav className="bg-white border border-[#e5e0d5] rounded-xl px-6 py-5 mt-8 flex gap-2.5 items-center">
          <Link href="/dashboard" className="bg-gray-100 px-3 py-1.5 rounded-full text-[13px] font-bold">← Дашборд</Link>
        </nav>
        ...
        <div className="bg-white border border-[#e5e0d5] rounded-2xl overflow-hidden mb-4 shadow-sm mt-8 px-6">
            <ExerciseItem id="ex1" num="1." problem="Yo _______ en el parque." correctAns="estoy" />
            <ExerciseItem id="ex2" num="2." problem="¿Dónde _______ tú?" correctAns="estás" />
            {/* ... remaining items ... */}
        </div>
      </div>
    </div>
