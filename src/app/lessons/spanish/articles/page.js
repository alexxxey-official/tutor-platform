...
export default function ArticlesLesson() {
  ...
  const total = 10
  const { progress, updateProgress, getStats, loading } = useLessonProgress('spa_articles', total, 0);
    ...
    return (
      <div className={`border-b border-dashed border-[#e5e0d5] py-4 flex gap-4 items-start last:border-none ${state.status === 'correct' ? 'bg-emerald-50/20' : ''}`}>
        ...
        <div className="flex-1">
          ...
          {isDone && <div className={`mt-1 text-[13px] font-medium ${state.status === 'correct' ? 'text-[#2a9d8f]' : 'text-[#e63946]'}`}>{state.status === 'correct' ? '✓ ¡Correcto!' : `✗ Correct: ${correctAns}`}</div>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f3] text-[#1a1a2e] pb-20 font-sans">
      <div className="bg-[#e63946] text-white px-10 py-12 pb-10">
        <h1 className="font-extrabold text-4xl mb-4 unbounded">Артикли и Род</h1>
      </div>
