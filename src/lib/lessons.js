export const LESSONS = {
  spanish: [
    {
      id: 'spa_intro',
      title: 'Введение: Карта грамматики',
      subject: 'Español',
      path: '/lessons/spanish/intro',
      color: '#e63946',
      totalCW: 0,
      totalHW: 0,
      totalScore: 0,
    },
    {
      id: 'spa_phonetics',
      title: 'Урок 1: Правила чтения (Фонетика)',
      subject: 'Español',
      path: '/lessons/spanish/phonetics',
      color: '#f4a261',
      totalCW: 13,
      totalHW: 10,
      totalScore: 23,
    },
    {
      id: 'spa_ser',
      title: 'Урок 2: Местоимения и глагол SER',
      subject: 'Español',
      path: '/lessons/spanish/ser',
      color: '#e63946',
      totalCW: 30,
      totalHW: 20,
      totalScore: 50,
    },
    {
      id: 'spa_articles',
      title: 'Урок 3: Артикли и Род',
      subject: 'Español',
      path: '/lessons/spanish/articles',
      color: '#e63946',
      totalCW: 30,
      totalHW: 20,
      totalScore: 50,
    },
    {
      id: 'spa_tener',
      title: 'Урок 4: Глагол TENER',
      subject: 'Español',
      path: '/lessons/spanish/tener',
      color: '#f4a261',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'spa_verbs',
      title: 'Урок 5: Правильные глаголы',
      subject: 'Español',
      path: '/lessons/spanish/verbs',
      color: '#f4a261',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'spa_estar',
      title: 'Урок 6: ESTAR и Предлоги',
      subject: 'Español',
      path: '/lessons/spanish/estar',
      color: '#f4a261',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'spa_questions',
      title: 'Урок 7: Вопросы и Порядок слов',
      subject: 'Español',
      path: '/lessons/spanish/questions',
      color: '#f4a261',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'spa_family',
      title: 'Урок 8: Семья и Внешность',
      subject: 'Español',
      path: '/lessons/spanish/family',
      color: '#f4a261',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'spa_gustar',
      title: 'Урок 9: Глагол GUSTAR и Еда',
      subject: 'Español',
      path: '/lessons/spanish/gustar',
      color: '#f4a261',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'spa_verbs_trainer',
      title: 'Тренажёр: 50 глаголов',
      subject: 'Español',
      path: '/lessons/spanish/verbs-trainer',
      color: '#e63946',
      totalCW: 0,
      totalHW: 50,
      totalScore: 50,
    },
    {
      id: 'spa_listening',
      title: 'Интерактивный Диктант',
      subject: 'Español',
      path: '/lessons/spanish/listening',
      color: '#e63946',
      totalCW: 15,
      totalHW: 10,
      totalScore: 25,
    },
    {
      id: 'spa_reading_hola',
      title: 'Чтение: Hola, me llamo...',
      subject: 'Español',
      path: '/lessons/spanish/reading/hola',
      color: '#f4a261',
      totalCW: 2,
      totalHW: 3,
      totalScore: 5,
    }
  ],
  english: [
    {
      id: 'eng_nobody',
      title: 'Nobody & Anyone (B1)',
      subject: 'English',
      path: '/lessons/english/indefinite-pronouns',
      color: '#2a9d8f',
      totalCW: 30,
      totalHW: 20,
      totalScore: 50,
    },
    {
      id: 'eng_passive',
      title: 'Passive Voice (B1-B2)',
      subject: 'English',
      path: '/lessons/english/passive-voice',
      color: '#2a9d8f',
      totalCW: 23,
      totalHW: 18,
      totalScore: 41,
    }
  ],
  math: [
    {
      id: 'math_quadratic',
      title: 'Квадратные уравнения',
      subject: 'Math',
      path: '/lessons/math/quadratic-equations',
      color: '#e63946',
      totalCW: 3,
      totalHW: 3,
      totalScore: 6,
    },
    {
      id: 'math_inequal',
      title: 'Неравенства',
      subject: 'Math',
      path: '/lessons/math/inequalities',
      color: '#e63946',
      totalCW: 6,
      totalHW: 8,
      totalScore: 14,
    }
  ],
  physics: [
    {
      id: 'phys_dc',
      title: 'Постоянный ток (DC)',
      subject: 'Physics',
      path: '/lessons/physics/dc-circuits',
      color: '#9333ea',
      totalCW: 2,
      totalHW: 2,
      totalScore: 4,
    }
  ],
  hebrew: [
    {
      id: 'heb_intro',
      title: 'Введение в иврит',
      subject: 'עברית',
      path: '/lessons/hebrew/intro',
      color: '#2563eb',
      totalCW: 0,
      totalHW: 0,
      totalScore: 0,
    },
    {
      id: 'heb_alphabet_1',
      title: 'Урок 1: Алфавит (א-כ)',
      subject: 'עברית',
      path: '/lessons/hebrew/alphabet-1',
      color: '#0891b2',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'heb_alphabet_2',
      title: 'Урок 2: Алфавит (ל-ת)',
      subject: 'עברית',
      path: '/lessons/hebrew/alphabet-2',
      color: '#0891b2',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'heb_vowels',
      title: 'Урок 3: Огласовки (Никуд)',
      subject: 'עברית',
      path: '/lessons/hebrew/vowels',
      color: '#2563eb',
      totalCW: 25,
      totalHW: 15,
      totalScore: 40,
    },
    {
      id: 'heb_reading',
      title: 'Урок 4: Правила чтения',
      subject: 'עברית',
      path: '/lessons/hebrew/reading',
      color: '#0891b2',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'heb_greetings',
      title: 'Урок 5: Приветствия',
      subject: 'עברית',
      path: '/lessons/hebrew/greetings',
      color: '#2563eb',
      totalCW: 25,
      totalHW: 15,
      totalScore: 40,
    },
    {
      id: 'heb_pronouns',
      title: 'Урок 6: Местоимения',
      subject: 'עברית',
      path: '/lessons/hebrew/pronouns',
      color: '#0891b2',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'heb_to_be',
      title: 'Урок 7: Глагол "быть"',
      subject: 'עברית',
      path: '/lessons/hebrew/to-be',
      color: '#2563eb',
      totalCW: 25,
      totalHW: 15,
      totalScore: 40,
    },
    {
      id: 'heb_numbers',
      title: 'Урок 8: Числа 1-10',
      subject: 'עברית',
      path: '/lessons/hebrew/numbers',
      color: '#0891b2',
      totalCW: 20,
      totalHW: 15,
      totalScore: 35,
    },
    {
      id: 'heb_family',
      title: 'Урок 9: Семья',
      subject: 'עברית',
      path: '/lessons/hebrew/family',
      color: '#2563eb',
      totalCW: 25,
      totalHW: 15,
      totalScore: 40,
    },
    {
      id: 'heb_verbs_present',
      title: 'Урок 10: Базовые глаголы',
      subject: 'עברית',
      path: '/lessons/hebrew/verbs-present',
      color: '#0891b2',
      totalCW: 30,
      totalHW: 20,
      totalScore: 50,
    },
  ]
};

// Функция для поиска урока по ID
export const getLessonById = (id) => {
  for (const subject of Object.keys(LESSONS)) {
    const lesson = LESSONS[subject].find(l => l.id === id);
    if (lesson) return lesson;
  }
  return null;
};
